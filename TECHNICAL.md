...
so `g0` does not exist for them; their 6 (of 50) Direction-A rows still count fully toward
...
gate on `_require_archive()` and SKIP with the reason named when this checkout lacks
`private/`.
