...
| `data/bill_gas_detail.csv` | long format (issue #98): `statement_date, period, charge_type (gas_service/gas_energy), segment, segment_days, baseline_rate, nonbaseline_rate, energy_rate` — one row per rate segment for EACH gas charge type. "Gas Service" (the tiered baseline/non-baseline rate) and "Gas Energy Charge" (a separate, flat, untiered $/therm charge on every therm) have INDEPENDENT segment counts within one period, so `charge_type` is the discriminator and only the columns relevant to it are populated. This is what a true marginal-tier gas rebilling needs and `bill_periods_gas.csv`'s blended columns cannot provide — see `heat_pump_conversion.py`'s `gas_savings_by_period()` for the reference consumer |
