...
# GENERATED TABLE: _CASE_FOLD_SED below is python's own full case folding,
# str.casefold(), emitted as a sed script. private_egress.py carries the
...
#
# run on the NEWEST interpreter available. str.casefold() is a property of
# that interpreter's Unicode version rather than a constant -- 1490 pairs under
# Unicode 13, 1530 under 15, 1557 under 16, measured -- so the table is PINNED
...
# SUPERSET of the running interpreter's str.casefold(), not equal to it: a pair
...
# the ones that fold to a sequence (sharp s to 'ss', U+0130 to 'i' plus a
# combining dot above, the Latin ligatures, the Greek letters carrying an iota
...
# Deseret), and the pairs simple lowercasing leaves apart (issue #234): final
# sigma / sigma, long s / 's', micro sign / mu, beta symbol / beta. A
# substitution carries a fold that changes the name's length as well as one
# that does not, and the two passes stay one function because no rule's
# output contains another rule's input (str.casefold() is idempotent; checked
# at generation). Two unrelated accents stay apart: 'i-acute' and 'a-ring' are
# not a case pair, and neither are U+0130 and 'i' or dotless i and 'i'.
#
...
#
# WHAT IT STILL DOES NOT FOLD, erring FAIL-OPEN and stated rather than
# implied: NFC/NFD normalization, which is a different axis, is joined by git's
# core.precomposeUnicode for the pathspec questions above, and is NOT joined
# for the tracked-but-absent entry this table exists for -- on a volume that
# resolves the two spellings to one file (APFS does, measured). Every pair the
# table carries was measured to be one file on this checkout's APFS volume,
# and the named negative controls two, so nothing else is known to be missing;
# that is a measurement on one volume rather than a derivation.
#
...
# generated from python's str.casefold() under Unicode 16.0.0
_CASE_FOLD_SED='
...
#           and the generated table has 131 pairs that do not (issue #233:
...
_gate_skips_entry() {   # $1 = a path relative to $DST_REAL   $2 = an index entry
  ...
  # unsound under C, where the 131 length-changing pairs move the count. Pinned
...
# the directory was looked at, never because a household staged it. Every
...
#
# THE COPY AND THE SCAN SHARE THIS ONE LIST, so they cannot drift apart
# (round 1 /review, issue #213): the copy phase below (THE COPY PHASE) never
# stages any of these four shapes from the source at all -- OS metadata is
# never household data, so it must not reach the destination in the first
# place, not merely be excused once it is already there. Staged and then
# excused would be worse than staged and refused: excused metadata is
# invisible to every future run of this very scan.
...
# OS METADATA IS NEVER COPIED (issue #213, round 1 /review): a source's own
# .DS_Store, AppleDouble sidecar, Thumbs.db or desktop.ini must never reach
# the destination, unconditionally -- unlike the scan's per-file exemption
# above, the copy has no "shadowed file present" question to ask, because
# nothing here is a LEFTOVER yet; it is what the source itself would hand the
# destination on an ordinary run, and OS metadata is never household data
# regardless of what else is beside it.
#
# Run on this run's OWN staging copy (STAGED COPY below), never on the
# source and never on an existing destination: `$1` is always a directory
# this run's own `cp -R` just finished writing, so pruning it is pruning a
# copy this run may freely discard, before the rename phase ever moves it
# into place. `-depth` so a directory is only tested after everything under
# it has been, which matters not at all here (nothing removed is a
# directory) but keeps this the same walk order `_collect_stale_under` uses.
_strip_os_metadata() {   # $1 = a staged directory this run just cp -R'd into
  local _entry
  find "$1" -depth -print0 2>/dev/null | while IFS= read -r -d '' _entry; do
    [ "$_entry" = "$1" ] && continue
    if _is_os_metadata "$(basename -- "$_entry")"; then
      rm -rf -- "$_entry"
    fi
  done
}
...
_strip_os_metadata "$_STAGE_RAW/electric-bills"
...
if [ "$HAS_GAS" = True ]; then
  ...
  _strip_os_metadata "$_STAGE_RAW/gas-bills"
fi
...
if [ "$STAGE_CAISO" = 1 ]; then
  ...
  _strip_os_metadata "$_STAGE_RAW/caiso_raw"
fi
