...
| `data/bill_gas_detail.csv` | Per period × charge type (Gas Service tiered baseline/non-baseline, or the flat Gas Energy Charge) × rate segment: each segment's own day count and $/therm rate(s), exactly as printed on each bill — what `bill_periods_gas.csv`'s blended columns collapse, and what a true marginal-tier gas rebilling needs |
