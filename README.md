...
| `data/battery_plan_matrix.json` | The §4 battery × plan matrix: no-battery / with-battery / battery-value per top-3 plan, plus the mid package (EV shift, then battery) re-billed end-to-end under each plan (table rates, cross-plan ranking; canonical-engine cross-checks included) |
