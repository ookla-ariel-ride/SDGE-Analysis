...
| `data/bill_corpus_boundary.json` | Where the electric bill corpus stops and why: the billing-history export that defines it, any statement parsed from a PDF but not published (with the reason and what would end the exclusion), and the resulting day coverage of the analysis window. Written by `analysis/parse_bills.py` in the same atomic set as the bill artifacts it describes |
