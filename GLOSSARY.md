...
**AB 205 fixed charge (income-graduated fixed charge)** — The 2022 California law, implemented by CPUC Decision D.24-05-028, that restructured residential electric bills around a flat monthly fixed charge (with income-based CARE/FERA discounts) offset by lower per-kWh rates. For SDG&E it took effect as the $24.15/month Base Services Charge billed from October 2025, and it is already in this report's bills and model. [CPUC Decision D.24-05-028](https://docs.cpuc.ca.gov/PublishedDocs/Published/G000/M531/K686/531686019.PDF)

**Avoided Cost Calculator (ACC)** — The CPUC's official hourly model of what a unit of customer generation saves the grid; it sets the export credit values under the Net Billing Tariff. This report prices its NEM 2.0 grandfathering value and the battery's NBT-era marginal value against SDG&E's hourly ACC-derived export-pricing table (`analysis/nem3_grandfathering.py`); a flat 3–8¢/kWh bracket remains in use only as a supplementary sensitivity check for the battery figures (`extended_findings.py`'s `nbt_2039`). [CPUC: Avoided Cost Calculator](https://www.cpuc.ca.gov/industries-and-topics/electrical-energy/demand-side-management/energy-efficiency/idsm)
...
**Grandfathering** — Being allowed to stay on an old, more favorable set of rules after the rules change for new customers. This home's solar is grandfathered on NEM 2.0 for about 20 years from its 2019 turn-on date. That status is worth **$2,103.58–$2,455.64 per year** versus the current Net Billing Tariff, re-billing the same measured year against SDG&E's hourly export-pricing table; the range reflects an unresolved question about this home's CCA generation credit (the household's usage data is not the source of the range). [CPUC: NEM and Net Billing](https://www.cpuc.ca.gov/industries-and-topics/electrical-energy/demand-side-management/customer-generation/net-energy-metering-and-net-billing)
...
**NBT (Net Billing Tariff) / Solar Billing Plan** — The rules for California solar systems connected since April 2023 (sometimes called "NEM 3.0"). Exports are credited at the grid's "avoided cost" (often 3–8¢/kWh) instead of near-retail rates, which is why keeping NEM 2.0 status matters. [CPUC: NEM and Net Billing](https://www.cpuc.ca.gov/industries-and-topics/electrical-energy/demand-side-management/customer-generation/net-energy-metering-and-net-billing)
...
**Netted export rate** — What an exported kWh is worth when it cancels an imported kWh in the same billing month and the same time-of-use period: the full energy price of that period (delivery plus generation plus PCIA), which in this home's weekday-midday super-off-peak cell is 10.4–10.6¢. It is the higher of the two ways a NEM 2.0 export can settle. The other is the surplus credit (7.6¢ summer, 7.8¢ winter in that same cell), paid only once a period's ledger has already gone net-negative for the month, so there is no import left to cancel. `analysis/rates.py` computes the two as `energy()` and `credit()`; §8 of the report brackets a whole year of exports between them, with the day-type assumption that bracket rests on stated there. The non-bypassable charge sits in neither: it rides on gross imports, which an export leaves untouched.
...
**PCIA (Power Charge Indifference Adjustment)** — A per-kWh "exit fee" CCA customers pay SDG&E to cover long-term power contracts the utility signed before they left. It keeps remaining bundled customers from bearing those legacy costs alone. It shows up as its own line only on CCA bills; bundled customers pay a PCIA-equivalent too, folded into the single bundled generation charge; a bundled statement says so in its own text ("$1.97 of your Electricity Generation Charge is your bundled PCIA charge"). [CPUC: PCIA](https://www.cpuc.ca.gov/industries-and-topics/electrical-energy/electric-costs/power-charge-indifference-adjustment)
...
**Rate vintage** — Which edition of the rate tables a dollar figure was computed with. This report's model uses June 2026 rates, while the actual bills were rendered largely on cheaper 2025 rates; the vintage gap explains why modeled totals run higher than the real bills; that gap is a tariff-date effect, not a modeling error.
...
**Therm** — The billing unit for natural gas, roughly the energy in 100 cubic feet of gas (about 29 kWh of heat). This home uses ~343 therms/yr, mostly for winter space heating. [EIA: energy units & calculators](https://www.eia.gov/energyexplained/units-and-calculators/)

**TOU (time-of-use)** — Any rate plan where the price per kWh depends on the time of day and season rather than being flat. The whole strategy of this report (charge the EV overnight or midday, avoid 4–9pm) comes from TOU price differences. [SDG&E pricing plans](https://www.sdge.com/residential/pricing-plans)
...
**Degradation** — The slow decline in panel output with age. Fitting this array's size-normalized yearly output for 2021–2025 gives a naive trend of 1.3–1.8%/yr of decline (`data/gross_import_decomposition.json`). That metric is not weather-normalized, and a single measured cleaning swung production 11.8%, so the true ageing rate is not determined here. The record does show no failing equipment and no abrupt step-down. See [§9 of the report](index.html#s9) for the full workup.
...
**Performance ratio (PR)** — The standard solar-industry normalization metric: actual AC energy delivered divided by what the array would deliver at its rated efficiency under the actual plane-of-array irradiance it received, beyond size and day count. It is what lets you compare a cloudy year to a sunny one on equal footing. This repo has no measured irradiance record for this array (no on-site pyranometer, no purchased satellite-irradiance feed), so a true PR is not determined here. `analysis/gross_import_decomposition.py` uses the best available substitutes: a deterministic Haurwitz clear-sky model (solar geometry only, no cloud cover) to normalize for day length and sun angle, day-matched calendar-window comparisons that use each year's own measured data to sidestep the missing irradiance series, and an empirically fitted ambient-temperature sensitivity as a magnitude check. Each is labeled when used. [Wikipedia: photovoltaic system performance](https://en.wikipedia.org/wiki/Photovoltaic_system_performance)
...
**Dispatch policy (evening-only / two-window / price-aware)** — The rules that decide *when* the battery charges and discharges. Evening-only covers just the 4–9pm peak; two-window adds the morning shoulder; price-aware discharges against every import priced above what the stored energy cost, and stores surplus only where the export it gives up is cheaper than the import that kWh can serve. Price-aware is worth ~$746/yr more than evening-only here ($2,466 against $1,720, `data/battery_dispatch_policies.json`), from settings alone.

**DSGS (Demand Side Grid Support)** — A California Energy Commission program that pays customers, including home-battery owners enrolled through their manufacturer's virtual power plant, for reducing load or exporting during summer grid emergencies. It is the one live battery revenue program in this territory, and it permits stacking with time-of-use bill savings. A household enrolls through one of ~14 anonymized VPP aggregations, so this report backtests it per aggregation: against the 2025 event calendar and this household's own measured load, a hypothetical Powerwall 3 would have net-earned **$97–$213** across the 14 individual aggregation schedules over the observed 2025-07-24–2025-10-30 partial season (not an annual figure; measured on the unconditional-charge dispatch rather than the published one, since re-measuring it needs a raw CEC event file this checkout does not carry; 20% reserve assumption; every 2025 event hour was a mandatory monthly test; July 2025 is excluded from revenue because its event hours straddle the measured window). The all-aggregations union figure, $138.52, sits inside that range. A full-season or annual figure is not determined, so the report treats DSGS as an amount added on top of the battery's own arbitrage payback and keeps it out of the payback-year figure. The CEC's 2026-season restriction applies to aggregators, not households: it freezes which VPP aggregators may participate (those active in October 2025) and caps each one's 2026 payment at its own October-2025 share of program funds, which discourages adding new sites without barring a household from joining an aggregator that already qualifies. Whether this household's own aggregator would accept a new enrollment is not determined from the public, anonymized CEC data. [CEC: Demand Side Grid Support Program](https://www.energy.ca.gov/programs-and-topics/programs/demand-side-grid-support-program)
...
**Knee (sizing curve)** — The smallest battery capacity at which the next kWh of storage stops paying back its marginal cost within the Powerwall 3's 10-year warranty term: past it, more storage is justified only for resilience. Found by sweeping capacity from 5 to 40 kWh and pricing each increment against a cost model fit to the two same-power Tesla configs. It lands at 20 kWh here in both the current and post-EV-fix scenarios; no shipping product sits exactly there (13.5 kWh undershoots it, 27 kWh overshoots it).

**Optimality gap** — The dollar (or percentage) difference between what a real dispatch policy saves and what the best possible controller could have saved on the same hardware and the same measured year: a bound on how much value the controller leaves unrealized. Computed here by solving the bill-minimizing schedule as a linear program and comparing it to the shipping price-aware policy's own saving: a $79.39/yr gap (3.2% of the shipping policy's own saving), so the shipping policy captures 96.9% of the theoretical maximum at this hardware.

**Perfect-foresight dispatch** — A battery schedule computed with full, error-free knowledge of an entire year's consumption and solar production in advance. It is the theoretical ceiling any controller on the same hardware could reach, used here to bound how much a smarter controller (or a real day-ahead forecast) is worth. Computed as a linear program that minimizes the exact monthly NEM-netting bill (a per-kWh price-arbitrage proxy can bill worse), verified by re-billing its own output through the same canonical billing engine used everywhere else in this report.
...
**Average emissions rate** — The grid-wide average kg CO₂ per MWh generated at a given time, computed from every generation source dispatched in that period; it is what `carbon_fullyear.py` computes from CAISO's public hourly CO₂-by-source and demand data. It answers "how clean is the grid mix right now"; the question "what happens if I add or remove one more kWh of demand" needs the marginal rate below. [CAISO emissions](https://www.caiso.com/todays-outlook/emissions)
...
**Grid carbon intensity** — How much CO₂ is emitted per unit of grid electricity (kg CO₂/MWh), which varies by hour. Measured from CAISO data across 364 of the analysis year's 365 days, the solar-midday window (10am–2pm) averages 109.1 kg CO₂/MWh against 270.1 overnight (midnight–6am), about 2.5× cleaner, comparing window average to window average; a single-hour comparison gives a different number (`data/carbon_fullyear_results.json`). [CAISO emissions](https://www.caiso.com/todays-outlook/emissions)

**Marginal emissions rate** — The emissions rate of the specific generating resource that would ramp up or down to meet one more (or one less) kWh of demand at a given moment: usually whichever fuel sits at the margin, often natural gas even in hours when the average mix looks clean (midday, when solar supplies most average generation but a load spike gets met by a gas peaker). This report does not compute a marginal rate, because CAISO's public "Today's Outlook" history endpoints, the only source `carbon_fullyear.py` fetches (`carbon_dispatch_tradeoff.py` only reads its already-committed output, `data/caiso_hourly_intensity.csv`), don't publish one. A marginal accounting would likely show the day/night carbon gap this report measures as larger, since gas is disproportionately the marginal resource at the overnight hours where this report already reports the dirtiest average intensity. [WattTime: what is marginal emissions](https://www.watttime.org/aer/what-is-marginal-emissions/)
...
**Electrification dividend** — This report's term for the money saved by driving on electricity instead of gasoline: the measured home-charging cost (plus estimated supercharging) versus what the same miles would cost at the cited state gasoline price and fleet fuel economy. It is about $3,191/yr here today, rising toward ~$4,429/yr once all charging lands super-off-peak.

**HPWH (heat-pump water heater)** — A water heater that moves heat from the surrounding air into the tank rather than making heat directly, using roughly a third the energy of a standard electric unit. On a super-off-peak timer it can run largely on cheap overnight power, with a second solar-coincident window on weekdays (10am–2pm). Most of the cheap running time is overnight, since super-off-peak on this tariff is 00:00–06:00 daily plus that midday slice. [DOE: heat pump water heaters](https://www.energy.gov/energysaver/heat-pump-water-heaters)

**Phantom load / always-on baseload** — Power the house draws around the clock even when "nothing" is on: refrigeration, pool pumps, chargers, standby electronics. This home's overnight floor is a median 1.03 kW, which held across the year costs about $3,157/yr priced against the hourly price map and $3,196/yr through a full NEM re-bill. A meter cannot tell you how much of that floor can be switched off, so the report measures the rate of return per 100 W removed.
...
**Simple payback** — Purchase price divided by annual savings: the years to break even, ignoring interest, discounting, and rate increases. A rough figure; this report quotes it alongside NPV.
...
**Counterfactual** — A "what if" version of the year (the same house with no solar, say, or with the EV charged at better times), re-billed under the same rules so the difference isolates one change's dollar value.
...
**Rate Relief Credit** — A per-kWh bill credit some CCAs apply to certain products. Whether it applies is read off the detailed bill; in this analysis the bills showed it does not apply to this account's product. [Clean Energy Alliance](https://thecleanenergyalliance.org)
