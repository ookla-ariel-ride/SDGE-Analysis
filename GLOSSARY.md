...
**Degradation** — The slow decline in panel output with age. Fitting this array's size-normalized yearly output for 2021–2025 gives a naive trend of 1.3–1.8%/yr of decline (`data/gross_import_decomposition.json`), but that metric is not weather-normalized and a single measured cleaning swung production 11.8%, so the true ageing rate is not determined here — what the record does show is no failing equipment and no abrupt step-down. See [§9 of the report](index.html#s9) for the full workup.
...
**Performance ratio (PR)** — The standard solar-industry normalization metric: actual AC energy delivered divided by what the array would deliver at its rated efficiency under the ACTUAL plane-of-array irradiance it received (not just size and day count). It is what lets you compare a cloudy year to a sunny one on equal footing. This repo has no measured irradiance record for this array (no on-site pyranometer, no purchased satellite-irradiance feed), so a true PR is not determined here — `analysis/gross_import_decomposition.py` uses the best available substitutes instead: a deterministic Haurwitz clear-sky model (solar geometry only, no cloud cover) to normalize for day length and sun angle, day-matched calendar-window comparisons that use each year's own measured data to sidestep the missing irradiance series, and an empirically fitted ambient-temperature sensitivity as a magnitude check. Each is labeled for what it actually is when used. [Wikipedia: photovoltaic system performance](https://en.wikipedia.org/wiki/Photovoltaic_system_performance)
...
**Optimality gap** — The dollar (or percentage) difference between what a real dispatch policy actually saves and what the best possible controller could have saved on the same hardware and the same measured year — a bound on how much value the CONTROLLER, not the battery, is leaving on the table. Computed here by solving the true bill-minimizing schedule as a linear program and comparing it to the shipping price-aware policy's own saving: a $217.39/yr gap (9.3% of the shipping policy's own saving) — the shipping policy already captures 91.5% of the theoretical maximum at this hardware.

**Perfect-foresight dispatch** — A battery schedule computed with full, error-free knowledge of an entire year's consumption and solar production in advance — not a real-world control strategy, but the theoretical ceiling any controller on the same hardware could reach, used here to bound how much a smarter controller (or a real day-ahead forecast) is worth. Computed as a linear program that minimizes the EXACT monthly NEM-netting bill (not a naive per-kWh price-arbitrage proxy, which can bill worse), verified by re-billing its own output through the same canonical billing engine used everywhere else in this report.
...
**Electrification dividend** — This report's term for the money already being saved by driving on electricity instead of gasoline: the measured home-charging cost (plus estimated supercharging) versus what the same miles would cost at the cited state gasoline price and fleet fuel economy — about $3,191/yr here today, rising toward ~$4,429/yr once all charging lands super-off-peak.
